...
func NewMessageStore() (*MessageStore, error) {
	...
	// Open SQLite database for messages. WAL lets the MCP server read while
	// the bridge writes without "database is locked" errors; the busy timeout
	// covers the brief write locks that remain.
...
func downloadMediaWithContext(ctx context.Context, client *whatsmeow.Client, messageStore *MessageStore, messageID, chatJID string) (bool, string, string, string, error) {
	...
	// Save the downloaded media to file, atomically and owner-only (0600).
...
func main() {
	...
	if !webhooksEnabled() {
		logger.Infof("Webhook disabled: WEBHOOK_URL is not set or WEBHOOK_ENABLED=false")
	} else {
